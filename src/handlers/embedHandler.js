...
const endStream = path.join(__dirname, '../../data/embedMessage.json');

// Helper function to read the JSON file
function readEmbedMessage() {
    try {
...
function writeEmbedMessage(data) {
    try {
...
async function fetchEmbedMessage(client, discordChannelId, messageId) {
    try {
...
async function statusUpdate(client) {
    ...
    while (true) {
        let statusMessage;
            try {
                statusMessage = readEmbedMessage();
            } catch (error) {
